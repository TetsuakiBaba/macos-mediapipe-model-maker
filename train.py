...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--learning-rate", default=0.001, type=float, help="Optimizer learning rate."
    )
    parser.add_argument(
        "--lr-decay", default=0.99, type=float, help="Learning-rate decay."
    )
    parser.add_argument(
        "--gamma", default=2, type=int, help="Focal loss gamma parameter."
    )
    parser.add_argument(
        "--dropout-rate", default=0.05, type=float, help="Classifier dropout rate."
    )
    parser.add_argument(
        "--layer-widths",
        default="",
...
def validate_dataset(dataset_dir: Path) -> list[tuple[str, int]]:
    ...
    if len(labels) < 2:
        raise ValueError('Add at least one gesture label in addition to "none".')
...
def parse_layer_widths(value: str) -> list[int]:
    ...
    for item in value.split(","):
        ...
        width = int(item)
        if width <= 0:
            raise ValueError("--layer-widths values must be positive integers.")
        widths.append(width)
    return widths
    ...
    class EpochLossLogger(tf.keras.callbacks.Callback):
        ...
        def on_epoch_end(self, epoch: int, logs: dict | None = None) -> None:
            ...
            if "categorical_accuracy" in logs:
                parts.append(f"acc={format_metric(logs.get('categorical_accuracy'))}")
...
def main() -> int:
    ...
    restore_training_hooks = install_epoch_loss_logger(gesture_recognizer, args.epochs)
